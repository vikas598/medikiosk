...
export const PatientConsentPage: React.FC = () => {
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-start px-4 py-4 max-w-4xl mx-auto w-full">
      {/* Top Header */}
      <div className="w-full flex items-center justify-between mb-3">
        <button
          onClick={() => navigate('/patient')}
          disabled={loading}
          className="flex items-center gap-2 bg-slate-200 hover:bg-slate-300 active:scale-95 text-slate-800 font-bold px-4 py-2 rounded-2xl shadow-sm text-base md:text-lg transition-all"
        >
          <ArrowLeft className="w-5 h-5" />
          <span>BACK</span>
        </button>

        <div className="flex items-center gap-2 bg-emerald-100 text-emerald-900 px-4 py-1.5 rounded-2xl font-bold text-sm md:text-base border border-emerald-300">
      ...
      {/* Main Consent Card */}
      <div className="kiosk-card mt-3 w-full p-5 md:p-7 rounded-[2rem] shadow-2xl border-2 border-slate-200 bg-white">
        <div className="flex items-center gap-3 mb-4 pb-4 border-b-2 border-slate-100">
          <div className="w-14 h-14 rounded-2xl bg-[#0C3B4A] text-white flex items-center justify-center">
          ...
          <div>
            ...
            <h2 className="text-2xl md:text-3xl font-black text-[#0C3B4A]">
              Welcome, {patientName}
            </h2>
          </div>
        </div>

        <div className="space-y-3 mb-5 text-slate-700 text-base md:text-lg leading-relaxed bg-slate-50 p-4 rounded-3xl border border-slate-200">
          <div className="flex items-start gap-3">
            <FileText className="w-6 h-6 text-[#00C9A7] shrink-0 mt-1" />
            <p>
          ...
          <p className="text-base text-slate-500 italic pl-9">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
          <button
            ...
            disabled={loading}
            className="h-20 md:h-24 bg-[#00C9A7] hover:bg-[#00A389] active:scale-98 text-slate-950 font-black text-2xl rounded-3xl shadow-xl transition-all flex items-center justify-center gap-3 border-b-6 border-teal-800"
          >
            {loading ? (
          ...
          <button
            ...
            disabled={loading}
            className="h-20 md:h-24 bg-slate-200 hover:bg-slate-300 active:scale-98 text-slate-800 font-bold text-xl rounded-3xl shadow-md transition-all flex items-center justify-center gap-3 border-b-4 border-slate-400"
          >
