...
export const KioskDocumentUploadPage: React.FC = () => {
  ...
  return (
    <div className="flex-1 flex flex-col justify-start items-center px-4 pb-4 w-full max-w-5xl mx-auto">
      {/* Top Header Controls */}
      <div className="w-full flex items-center justify-between mb-4 z-10">
        <button
          type="button"
          onClick={() => navigate(-1)}
          className="flex items-center gap-2 bg-slate-200/80 hover:bg-slate-300 active:scale-95 text-slate-800 font-bold px-4 py-2 rounded-2xl shadow-sm text-base md:text-lg transition-all"
        >
          <ArrowLeft className="w-7 h-7" />
          <span>BACK</span>
        </button>

        <div className="flex items-center gap-2 bg-emerald-500/20 text-emerald-300 px-4 py-2 rounded-2xl border border-emerald-500/30 shadow-sm font-bold text-base">
      ...
      {/* Main Kiosk QR Card */}
      <div className="bg-white rounded-[2rem] p-5 md:p-7 shadow-2xl max-w-3xl w-full text-center space-y-5 border border-slate-100 mt-3">
        
        {/* Title & Large Icon */}
        <div className="space-y-2">
          <div className="w-16 h-16 rounded-full bg-gradient-to-br from-emerald-400 to-teal-600 text-white mx-auto flex items-center justify-center shadow-lg mb-3">
            <FileUp className="w-8 h-8 stroke-[2.5]" />
          </div>

          <h1 className="text-2xl md:text-3xl font-extrabold text-[#0A1926] tracking-tight">
            Upload Medical Documents
          </h1>

          <p className="text-base md:text-lg text-[#0D9488] font-bold">
        ...
        {/* LARGE QR CODE BOX */}
        {handoffToken ? (
          <div className="bg-[#F8FAFC] border-2 border-dashed border-teal-500/40 p-4 md:p-5 rounded-3xl inline-block shadow-inner">
